...
import {
  Box, Button, Typography, Menu, Checkbox,
  FormControlLabel, Divider, TextField, InputAdornment, IconButton
} from '@mui/material';
...
const FilterDropdown = ({ filter, options, selectedOptions, onOptionChange, searchEnabled = true, onReset }) => {
        ...
        <Box sx={{ padding: '15px 12px', width: '280px', }}>
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography sx={{ fontSize: '14px', fontWeight: '500', margin: "0px 10px", paddingTop: "9px", paddingBottom: "0px" }}>All Airlines</Typography>
              {
                selectedOptions.length > 0 && (
                  <Button onClick={onReset} sx={{ fontSize: '0.9rem', color: '#FFBF00', textTransform: 'none', }}>Reset</Button>
                )
              }
            ...
            {selectedOptions.length > 0 ? (
              selectedOptions.map((option, idx) => (
                <FormControlLabel
                  key={idx}
                  control={<CustomCheckbox checked value={option} onChange={handleOptionChange}
                    sx={{ marginX: '10px' }}
                  />}
                  label={option}
...
const AirlinesFilter = () => {
  ...
  const optionsAlliances = [
    'Aerospatiale/Alenia ATR 42-300', 'Aerospatiale/Alenia ATR 72', 'Agusta / AgustaWestland A-109',
    'Airbus A220', 'Airbus A320-100'
  ];
